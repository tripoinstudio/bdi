...
if ($_GET['action'] == 'search') {
    $dblist = new Database();
    $dblist->connect();
    $dblist->sql('SELECT * FROM `tb_personal_identity` s JOIN `tb_address` a ON s.`tb_personal_identity_ktp_address`=a.`tb_address_id` WHERE a.`tb_address_id`=' . $_GET['address']); // Table name, Column Names, JOIN, WHERE conditions, ORDER BY conditions
    $list_query = $dblist->getResult();

    $length_list = count($list_query);
    //    echo $_GET['address'];
    include 'component/' . $_GET['content'] . '/' . $_GET['content'] . '_list.html.php';
