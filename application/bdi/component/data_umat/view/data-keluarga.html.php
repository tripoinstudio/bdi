...
<?php
$no=0;
$dbumat->sql('SELECT r.`tb_relationship_name`,r.`tb_data_umat_id`,r.`tb_relationship_relation_code`,f.`tb_family_relationship_type` FROM `tb_relationship` r INNER JOIN `tb_family_relationship` f ON r.`tb_relationship_relationship_id`=f.`tb_family_relationship_id` WHERE f.`tb_family_relationship_id`='.$query1['tb_data_umat_hub1']); // Table name, Column Names, JOIN, WHERE conditions, ORDER BY conditions
...
foreach ($list_keluarga as $array_keluarga) {
	...
	<span style="padding-left: 30px;"><?=idListViewTarget($array_keluarga['tb_data_umat_id'],'data_umat','tb_data_umat_nama_ktp');?></span>
     </label>    
...
foreach ($list_keluarga as $array_keluarga) {
	...
	<div class="form-row control-group row-fluid">
    <label class="control-label span4 ">Nama 
	<span style="padding-left: 30px;"><?=idListViewTarget($array_keluarga['tb_data_umat_id'],'data_umat','tb_data_umat_nama_ktp');?>
	</span>
     </label>    
